...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Get User PDF Forms Servlet",
		...
		"sling.servlet.selectors=" + "userforms", "sling.servlet.extensions=" + "json" })
public class GetUserFormsServlet extends SlingAllMethodsServlet {
	...
	protected void doGet(final SlingHttpServletRequest request, final SlingHttpServletResponse response)
		...
		try {
			...
			response.setCharacterEncoding("UTF-8");
			out.print(userFormService.userFormsJson(request));
			out.flush();
