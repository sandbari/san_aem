...
/**
 * Adapts resources to {@link ContentFragment} objects by mocking parts of their
 * API.
 */
public class ContentFragmentMockAdapter implements Function<Resource, ContentFragment> {
	...
	public ContentFragment apply(@Nullable Resource resource) {
		...
		if (isStructured) {
			...
			// for the 'adaptTo' mock below we use the jcr:content child to
			// mimick the real behavior
	...
	/**
	 * Creates a mock of a content element for a text-only (if {@code model} is
	 * {@code null}) or structured (if {@code model} is not {@code null})
	 * content fragment.
	 */
	...
	/**
	 * Collects and returns the information of a content element for text-only
	 * content fragment.
	 */
	private MockElement getTextOnlyElement(Resource resource, String name) {
		...
		// loop through element definitions in the model and find the matching
		// one
		boolean found = false;
		...
		try {
			...
			for (Resource rendition : renditions.getChildren()) {
				...
				if (variation != null) {
					...
					element.addVariation(rendition.getName(), title, contentType, new String[] { content }, true,
							content);
				} else {
	...
	/**
	 * Collects and returns the information of a content element for structured
	 * content fragment.
	 */
	private MockElement getStructuredElement(Resource resource, Resource model, String name) {
		...
		// loop through element definitions in the model and find the matching
		// one (or first one, if name is null)
		boolean found = false;
		...
		for (Resource item : items.getChildren()) {
			...
			if (element.name == null || element.name.equals(elementName)) {
				...
				// determine if the element is multi-valued (if the value type
				// is e.g. "string[]")
