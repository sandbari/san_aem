...
(function(document, $) {
    ...
    $(document).on("foundation-contentloaded", function(e) {
        ...
        var url= "/content/vcm/banner.enabledbanner.json";


     */
/* $.getJSON( url, function( data ) {
            if (data == true) {
                if ($(".cq-RichText-editable[name='./mobileMessage']").parent().attr("checked") != "checked") {
                    $(".coral3-Checkbox-input[name='./enabledBanner']").attr('disabled','disabled');
                }
            }
        });*/
/*
    ...
    registry.register("foundation.validation.validator",{
        ...
        selector : "[data-validation=mobile-message-validator]",
        validate : function(element){

            console.log("test  "+ $(element).text());

            var regex = /(<([^>]+)>)/ig;

            var result = $(element).text().replace(regex, "");
