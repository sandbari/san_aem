                    ...
                    if (allDepPanelIds.indexOf(ariaControls) !== -1) {
                        if (negate) {
                            ...
                            if (ariaControls !== tabId) {
                                //code to restrict validation of hidden required field
                                //$('#'+ariaControls).find('[aria-required=true]').attr('aria-required','false');
                                tabItem.hidden = !visibaleTabIds.includes(ariaControls)
                            } else {              
                                //code to restrict validation of hidden required field
                                //$('#'+ariaControls).find('[aria-required=false]').attr('aria-required','true');
